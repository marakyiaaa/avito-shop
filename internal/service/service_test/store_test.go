...
func TestStoreService_BuyItem_Success(t *testing.T) {
	...
	mockItemRepo.On("GetItemByName", mock.Anything, "cup").Return(expectedItem, nil)
	...
	mockInventoryRepo.On("AddToInventory", mock.Anything, 1, "cup").Return(nil)

	storeService := service.NewStoreService(mockUserRepo, mockItemRepo, mockInventoryRepo)
	err := storeService.BuyItem(context.Background(), 1, "cup")
...
func TestStoreService_BuyItem_AddToInventoryError(t *testing.T) {
	...
	mockUserRepo.On("GetUserByID", mock.Anything, 1).Return(expectedUser, nil)
	mockUserRepo.On("UpdateUserBalance", mock.Anything, 1, 900).Return(nil)
	mockInventoryRepo.On("AddToInventory", mock.Anything, 1, "cup").Return(fmt.Errorf("ошибка при добавлении в инвентарь"))
